  ...
  useEffect(() => {
    ...
    return (
        ...
        <p>
          ...
          1: Individuals’ total pay breakdown is broken down into regular, overtime, detail, injury, Quinn, and other. <br/>
          2: The following visualization can then select one portion of the pay and compare it to the rest. <br/>
        ...
        <p>
          When we first began to analyze our researched datasets, we quickly noticed that only one would be of any use; the other four were some combination of redundant, too small, or downright useless for the scale of our project. The PCIPP dataset, for example, contains what is essentially our only clear data on Quinn payments — but it’s only a sample of 141 entries. We decided that the only data we could truly analyze with any statistical confidence was the 2011-2020 earnings report, as it contained over 30,000 entries of relevant police financial information despite being painfully disorganized.
          <br/><br/>
          Cleaning the data was especially demanding for this project as our primary dataset was largely unclean. After clearing oddities such as invalid characters and null values, we had to find some way to deal with the countless names that different teams assign their officers, including stylistic differences such as “Police Offc.” vs. “Police Officer” and various role modifiers that complicated the merging process. In order to construct a solution, we first met as a team to discern which specific roles we wanted to represent in our visualizations. After analyzing the data, we settled on 13 distinct titles to categorize officers to both allow for an effective visualization without miscategorizing or misrepresenting certain positions. We included the distinction between an officer role and detective role for a given rank as there was evidence of a significant difference in salary between the two. Finally, with some pandas scripting and data prepping, we iterated through the dataset, sorting each entry by role and taking the average of their yearly total earnings. From this standpoint, we were able to more easily modify our existing code to make new visualizations including individual salary breakdowns and year-on-year average salary charts.
          <br/><br/>
          For the graph which analyzes individuals’ total pay breakdown, we cleaned the data by removing records without employee_id, converting salary information from string to integer, and removing any extra spacings and commas.
        </p>
